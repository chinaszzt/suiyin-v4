...
- **I9（安全闸）**: 以下三条规则任一命中即 `SAFETY_BLOCKED`：(1) 测试/验证命令或新增 diff 行**以连接/指向形态**出现生产 MongoDB 端口 `27017`（`host:27017` / `--port 27017` / `port=27017`——v0.6.0 收紧：守卫代码/文档"提到禁令"不再命中，desk 实证 33 处误报归零而指向形态全保）；(2) 同一行内生产库账号 `bzds` 与写操作词共现（只读放行）；(3) 新增 diff 行含 private key、AWS access key（全大写 `AKIA…`）、OpenAI 风格密钥（`sk-`+纯字母数字体，路径片段不误中）或非占位符明文凭证。输入闸在 worktree/session 之前阻断，diff 闸在 commit 采纳、push/开 PR 前阻断并保留 worktree；全程零模型参与，仅用确定性正则扫描。**file-aware（v0.6.0）**：规则 4 命中的 `.suiyin/` 工件文件整体已拦，其内容行不再重复过规则 1-3。**行内豁免**：守卫代码自身必须含违禁字面量时标 `safety-ok: <说明>` 跳过该行（标注留在 diff 里 C5 可审计；威胁模型不含 AI 主动破坏）。
...
**Changelog**:
- v0.5.2 (2026-08-13): **PATCH — M3 件 8 安全闸误报面校准**（I9 措辞更新，schema 不变）。desk 真 diff 73 处误报三层解剖归零：(a) file-aware——`.suiyin/` 工件文件（规则 4 已整体拦截）的内容行不再重复过规则 1-3（50/73，session log 内容逐行误报）；(b) 27017 收紧为连接/指向形态 `host:27017`/`--port 27017`/`port=27017`（7 处守卫代码/文档"提到禁令"误中归零，verify_cmd 指向精度 = desk 旧机制）；(c) 凭证 regex 收紧——AWS 全大写、`sk-` 体纯字母数字（16 处路径片段 `sk-v4lab-worktrees-…` + 2 处混合大小写随机串归零）；(d) 新增 `safety-ok:` 行内豁免（可审计，C5 看得到标注）。验收：desk 真 diff 重跑 = 仅剩规则 4 的 3 处真问题；TP 敏感度探针全保。evidence: `dogfood/P0-attribution/`。
